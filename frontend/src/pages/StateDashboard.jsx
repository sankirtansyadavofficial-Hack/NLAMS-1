...
function StateMap({ currentState, getDynamicDistrict }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://raw.githubusercontent.com/geohacker/india/master/district/${formattedStateName}.geojson`)
      ...
      .then(setGeoData)
      .catch(() => {
        // Fallback to MH if not found just to show something
  ...
  const districtActivityColor = (name) => {
    ...
    // If not found in dynamic mapping, just assign a random color based on string length to simulate data
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <select
                  ...
                  className="bg-transparent text-xs font-bold text-white leading-tight focus:outline-none cursor-pointer hover:text-amber-400 transition-colors appearance-none"
                ...
                >
                  {[
                    ...
                    <option key={state} value={state} className="bg-[#0D1F0A] text-white">
                      {state} Government
                    </option>
                  ))}
                </select>
                <div className="text-[10px] text-white/40">Revenue Dept</div>
              </div>
            </div>
          </div>
