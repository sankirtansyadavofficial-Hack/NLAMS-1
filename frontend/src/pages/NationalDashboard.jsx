...
import {
  stateData, activityFeed,
  getKPITotals, getTop10StatesByArea
} from '../data/dashboardData';
import { getAllParcels, searchParcel, getDashboardStats, getAllProjects } from '../services/api';
...
function PlotSearchMap() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const data = await searchParcel(searchPlot);
  ...
  return (
    <div className="glass rounded-2xl p-6 border border-white/10 mt-8 mb-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3">
          <input 
            ...
            className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white focus:border-amber-400 focus:outline-none placeholder-white/30"
          />
          <button 
            onClick={handleSearch}
            className="px-6 py-2 rounded-xl font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 transition-all"
          >
            Search
          </button>
        </div>
      ...
      {error && <div className="text-red-400 text-sm font-bold mb-4 bg-red-500/10 p-3 rounded-lg border border-red-500/20">{error}</div>}

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 h-96 rounded-xl overflow-hidden border border-white/10 relative">
          <MapContainer
          ...
          >
            ...
            {searchedData && searchedData.latitude && searchedData.longitude && (
              <MapUpdater center={[searchedData.latitude, searchedData.longitude]} zoom={17} />
            )}
            {geoData && (
              <GeoJSON 
                key={searchedData ? searchedData.plot_number : 'default-map'}
                data={geoData} 
                style={(feature) => {
                  let color = '#f59e0b'; // default
                  const usage = (feature.properties.usage || '').toLowerCase();
                  if (usage.includes('residential')) color = '#3b82f6';
                  else if (usage.includes('agriculture')) color = '#22c55e';
                  else if (usage.includes('government')) color = '#f97316';
                  else if (usage.includes('dispute') || usage.includes('encroach')) color = '#ef4444';
                  
                  const isSearched = searchedData && feature.properties.plot_number === searchedData.plot_number;
                  
                  return {
                    color: isSearched ? '#ffffff' : color, // Highlight border
                    weight: isSearched ? 4 : 2, // Thicker border
                    fillColor: color,
                    fillOpacity: isSearched ? 0.9 : 0.5 // Higher opacity
                  };
                }}
                onEachFeature={(feature, layer) => {
                  layer.bindPopup(`
                    <div style="font-family: sans-serif; color: #333;">
                      ...
                      Khasra: ${feature.properties.khasra_number || 'N/A'}<br/>
                      Mutation: ${feature.properties.mutation_status || 'N/A'}<br/>
        ...
        <div className="lg:col-span-1 space-y-4 overflow-y-auto max-h-[24rem]">
          <div className="p-4 rounded-xl bg-white/5 border border-white/10 min-h-full">
            <h3 className="text-sm font-bold text-white/50 uppercase tracking-widest mb-4">Plot Details</h3>
            {searchedData ? (
              <div className="space-y-4">
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Plot Number</div>
                  <div className="text-lg font-black text-amber-400">{searchedData.plot_number}</div>
                </div>
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Coordinates (Lat, Lng)</div>
                ...
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Owner Name</div>
                  <div className="text-sm font-bold text-white">{searchedData.owner_name}</div>
                </div>
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Contact</div>
                  <div className="text-xs text-white/80">{searchedData.owner_phone}</div>
                </div>
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Aadhaar (Masked)</div>
                  <div className="text-xs font-mono text-white/80">{searchedData.owner_aadhaar}</div>
                </div>
                <div>
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Address</div>
                  <div className="text-xs text-white/70 leading-tight">{searchedData.owner_address}</div>
                </div>
                <div className="pt-2 border-t border-white/10">
                  <div className="text-[10px] text-white/40 uppercase mb-0.5">Current Valuation</div>
                  <div className="text-base font-black text-emerald-400">₹{searchedData.valuation.toLocaleString('en-IN')}</div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <div className="text-[10px] text-white/40 uppercase mb-0.5">Khasra No.</div>
                    <div className="text-xs font-bold text-white">{searchedData.khasra_number || 'N/A'}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-white/40 uppercase mb-0.5">Khata No.</div>
                    <div className="text-xs font-bold text-white">{searchedData.khata_number || 'N/A'}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-white/40 uppercase mb-0.5">Mutation Status</div>
                    <div className="text-xs font-bold text-white">{searchedData.mutation_status || 'N/A'}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-white/40 uppercase mb-0.5">Land Usage</div>
                    <div className="text-xs font-bold text-white">{searchedData.usage}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-white/40 uppercase mb-0.5">Area</div>
                    <div className="text-xs font-bold text-white">{searchedData.parcel_area} acres</div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-white/30 text-sm flex h-4/5 items-center justify-center text-center mt-10">
...
export default function NationalDashboard() {
  ...
  const initialKpi = useMemo(() => getKPITotals(), []);
  ...
  useEffect(() => {
    getDashboardStats().then(data => {
      setKpi(prev => ({
        ...prev,
        totalNotified: prev.totalNotified, // keeping mock notified
        totalAcquired: data.totalArea,
        compensationSanctioned: prev.compensationSanctioned,
        compensationDisbursed: (data.totalValuation / 10000000).toFixed(1), // convert ₹ to Cr
        familiesAffected: prev.familiesAffected
      }));
  ...
  }, [apiProjects, selectedState, statusFilter, searchTerm]);
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 space-y-8">
        ...
        >
              ...
              <div>
                <h2 className="text-lg font-black text-white flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-white/40 ml-2">
                    ({filteredProjects.length} of {apiProjects.length})
                  </span>
                </h2>
              </div>
          ...
          <div className="px-5 py-4 border-t border-white/5 flex items-center justify-between text-xs text-white/30">
            <span>Showing {filteredProjects.length} of {apiProjects.length} projects</span>
